...
export default function NavbarAnnouncementBadge() {
  ...
  return (
    <Link
      ...
      className="relative p-2 rounded-xl text-gray-600 hover:text-gray-900 hover:bg-gray-100 transition-colors focus:outline-none focus:ring-2 focus:ring-sky-500/20 group"
      title={t("title")}
      aria-label={t("title")}
    >
      <Bell className="w-5 h-5 transition-transform group-hover:scale-110 text-gray-700" />
      {unreadCount > 0 && (
        <span className="absolute -top-0.5 -right-0.5 rtl:-left-0.5 rtl:right-auto flex items-center justify-center">
