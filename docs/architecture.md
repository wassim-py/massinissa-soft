...
### 7.22 Student activation grace period + trailing-absence payroll/refund hold (new — refines §1.5, §2.9, §7.8)

This tightens two pieces of the credit/payroll logic that were previously under-specified. It does **not** change how a group itself starts (§1.4/§1.5: a class's lessons stay inert, no credit touched, until the admin takes attendance for that group for the first time) — it adds two new layers on top of that, both scoped **per `Enrollment`**, not per group.

**A. Per-student activation grace period (freshly-started groups only)**
- Once a group has started (first attendance ever taken for that class), an individual `Enrollment` still isn't "live" for credit purposes until that specific student's own first `PRESENT` in that group. Track this as `Enrollment.activatedAt` (null until set).
- While `activatedAt` is null, any `ABSENT` marked for that student in that group is logged as history but **does not** touch session credit, is **excluded from teacher payroll**, and is **never refund-relevant** (nothing was ever charged).
- The moment the student is first marked `PRESENT`, `activatedAt` is stamped and the grace period ends **permanently** for that enrollment — every lesson from that point on (present or absent) consumes credit and behaves normally, including the very first lesson of any later renewal cycle within this same enrollment.
- This grace period only exists for the group's just-started state. A student joining an already-running group, or already activated, gets no grace window — attendance behaves as it always has from day one.
- **Amends the `Attendance` model comment (§3)**: correct the stale note that credit is decremented "on PRESENT" only — outside the grace window above, both `PRESENT` and `ABSENT` consume session credit; only `NOT_DEFINED` and pre-activation absences are excluded.

**B. Trailing-absence hold at month-end payroll (activated enrollments only)**
Applies only once a student's grace period (if any) is over — i.e. to normal, already-activated attendance.

- Teacher payroll (§2.9) is computed **lesson by lesson**, monthly, on the 1st (`PayrollRun.periodStart/periodEnd`) — already the intended model (`Attendance`-driven, not voucher-driven); this section adds the one case where a lesson is deliberately excluded from that computation.
- For each `Enrollment`, look at the trailing run of consecutive `ABSENT` records immediately preceding the payroll cutoff — i.e. the student's most recent attendance activity in that group is an unbroken run of absences with no `PRESENT` after it and no resolution yet. Every lesson in that trailing run is `HELD`:
  - Excluded from this payroll run's teacher pay.
  - Excluded from that month's finalized net-profit figures (owner dashboard, §1.1/§1.3).
  - Refund-eligible for the student — on top of, not replacing, the existing cashback rule (§7.8), which is capped by unconsumed credit; a `HELD` lesson is refundable specifically **because** it's unresolved, regardless of the cashback cap.
- Track this as `Attendance.payrollStatus` (`SETTLED` default | `HELD` | `DEFERRED_PAID` | `VOID_REFUNDED`) so it can be queried/reported independently of the run-time trailing-run scan.
- **Resolution — student attends again**: the moment the student is marked `PRESENT` in that group again (whether that's simply the next lesson on remaining credit, or the first lesson of a newly paid cycle after credit ran out — confirmed: a new cycle purchase + attendance counts as "attending again"), every currently-`HELD` record for that enrollment flips to `DEFERRED_PAID`: the student is charged and non-refundable for all of them as normal, and the teacher is paid for all of them in the **next** `PayrollRun` — a closed month's run is never reopened/back-paid.
- **Resolution — student is refunded/transferred instead**: if a cashback/refund or `EnrollmentTransfer` is recorded for the enrollment before the student attends again, every currently-`HELD` record flips to `VOID_REFUNDED` — refunded to the student, and the teacher is never paid for them.
- Until one of those two resolutions happens, the `HELD` batch just carries forward untouched — it can span more than one payroll cutoff if the student stays away for multiple months.
- This rule is entirely about the **trailing, still-open** run. Any earlier absence in the same month followed by at least one later `PRESENT` before the cutoff is **not** held — it's `SETTLED` immediately: charged, non-refundable, and paid to the teacher in that same month's run, exactly as before (this is what Examples 1–3 in the owner's spec already describe, unchanged).
- `NOT_DEFINED` records are unaffected either way — still fully excluded from credit/payroll/refund — and are treated as pass-through when scanning for the trailing run: a `NOT_DEFINED` in the middle of an absence streak doesn't break the run and isn't itself held.
- Interacts cleanly with `isFree` and the payer-status flag (§7.18): those exclusions apply first, so the hold logic never needs to act on a lesson that was never going to be paid to begin with.

**Flag for Wassim to confirm with Antigravity** (two edge cases, not blocking — the addendum picks a default for each):
1. Does a `CatchUpAttendance` (§2.12) — the student attending a *different* group's lesson to make up a miss — count as "attending again" for resolving a `HELD` run in their own group's enrollment? Default: **no** — only a `PRESENT` in the student's own enrollment/group resolves the hold, since credit/payroll for the held lessons is tied specifically to that enrollment.
2. If a student's `HELD` run later resolves as `VOID_REFUNDED`, does that also roll back their `activatedAt` if it was set during that same trailing run? Default: **no** — `activatedAt`, once set, stays permanent per part A; a refund only reverses the specific `HELD` lessons' charge/payroll status, not activation.
